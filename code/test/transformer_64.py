...
class Generator(nn.Module):
    ...
    def forward(self, src, mask):
        ...
        out_c, score_c = self.decoder(embedding[torch.where(in_mask[:]==1)[0],torch.where(in_mask[:]==1)[1]].reshape(embedding.shape[0],-1,embedding.shape[-1]),
                                         memory)    
        for u in range(int(order[0,63].data) + 1,65): 
...
class En_De_Attention(nn.Module):
    ...
    def forward(self, x, y,score_c,tt,out_c):
        ...
        k = self.key(y).view(B, T1, self.n_head, C // self.n_head).transpose(1, 2)  # (B, nh, T, hs)
        v = self.value(y).view(B, T1, self.n_head, C // self.n_head).transpose(1, 2)  # (B, nh, T, hs)
